	;; $Id: memcpyset.s,v 1.3 1994/04/08 18:16:02 limes Exp $
...
memcpy
	...
	BLO	CopyDown
 [	{CONFIG}=26
	MOVEQS	pc, lr			; dst == src, no move required
 |
	MOVEQ	pc, lr			; dst == src, no move required
 ]
...
Up_TrailingBytes
 ...
 |
	...
	STRGTB	tmp1, [dst], #1
 [	{CONFIG}=26
	LDMFD	sp!, {a1, pc}^		 ; recover old dst value
 |
	LDMFD	sp!, {a1, pc}		 ; recover old dst value
 ]
...
0	MOV	tmp1, lr, LSL #8	; first three bytes	; 1
	LDMIA	src!, {v1, v2, tmp3, lr}			; 12/13
	ORR	tmp1, tmp1, v1, LSR #24 	; word 1	; 1
	MOV	v1, v1, LSL #8					; ...
	ORR	v1, v1, v2, LSR #24		; word 2	; 2 (1+1)
	MOV	v2, v2, LSL #8
	ORR	v2, v2, tmp3, LSR #24		; word 3	; 2
	MOV	tmp3, tmp3, LSL #8
	ORR	tmp3, tmp3, lr, LSR #24 	; word 4	; 2
...
1	MOV	tmp3, lr, LSL #8	; first three bytes	; 1
	LDR	lr, [src], #4		; next four bytes	; 4
	ORR	tmp3, tmp3, lr, LSR #24 			; 1
	STR	tmp3, [dst], #4 				; 4
	SUBS	n, n, #4					; 1
...
0	MOV	tmp1, lr, LSL #16	; first two bytes
	LDMIA	src!, {v1, v2, tmp3, lr}
	ORR	tmp1, tmp1, v1, LSR #16 ; word 1
	MOV	v1, v1, LSL #16
	ORR	v1, v1, v2, LSR #16	; word 2
	MOV	v2, v2, LSL #16
	ORR	v2, v2, tmp3, LSR #16	; word 3
	MOV	tmp3, tmp3, LSL #16
	ORR	tmp3, tmp3, lr, LSR #16 ; word 4
	...
	BLT	%F2
1	MOV	tmp3, lr, LSL #16	; first two bytes
	LDR	lr, [src], #4		; next four bytes
	ORR	tmp3, tmp3, lr, LSR #16
	STR	tmp3, [dst], #4
	SUBS	n, n, #4
...
0	MOV	tmp1, lr, LSL #24	; first byte
	LDMIA	src!, {v1, v2, tmp3, lr}
	ORR	tmp1, tmp1, v1, LSR #8	; word 1
	MOV	v1, v1, LSL #24
	ORR	v1, v1, v2, LSR #8	; word 2
	MOV	v2, v2, LSL #24
	ORR	v2, v2, tmp3, LSR #8	; word 3
	MOV	tmp3, tmp3, LSL #24
	ORR	tmp3, tmp3, lr, LSR #8	; word 4
	STMIA	dst!, {tmp1, v1, v2, tmp3}
	SUBS	n, n, #16
	BGE	%B0
	; Reload registers
	...
	ADDS	n, n, #16-4		; check number of bytes
	BLT	%F2
1	MOV	tmp3, lr, LSL #24	; first byte
	LDR	lr, [src], #4		; next four bytes
	ORR	tmp3, tmp3, lr, LSR #8
	STR	tmp3, [dst], #4
	SUBS	n, n, #4
...
0	MOV	lr, tmp1, LSR #8	; last three bytes
	LDMDB	src!, {tmp1, v1, v2, tmp3}
	ORR	lr, lr, tmp3, LSL #24	; word 4
	MOV	tmp3, tmp3, LSR #8
	ORR	tmp3, tmp3, v2, LSL #24 ; word 3
	MOV	v2, v2, LSR #8
	ORR	v2, v2, v1, LSL #24	; word 2
	MOV	v1, v1, LSR #8
	ORR	v1, v1, tmp1, LSL #24	; word 1
...
1	MOV	tmp3, tmp1, LSR #8	; last three bytes
	LDR	tmp1, [src, #-4]!	; previous four bytes
	ORR	tmp3, tmp3, tmp1, LSL #24
	STR	tmp3, [dst, #-4]!
	SUBS	n, n, #4
...
0	MOV	lr, tmp1, LSR #16	; last two bytes
	LDMDB	src!, {tmp1, v1, v2, tmp3}
	ORR	lr, lr, tmp3, LSL #16	; word 4
	MOV	tmp3, tmp3, LSR #16
	ORR	tmp3, tmp3, v2, LSL #16 ; word 3
	MOV	v2, v2, LSR #16
	ORR	v2, v2, v1, LSL #16	; word 2
	MOV	v1, v1, LSR #16
	ORR	v1, v1, tmp1, LSL #16	; word 1
	STMDB	dst!, {v1, v2, tmp3, lr}
	SUBS	n, n, #16
	BGE	%B0
	; Reload registers
...
1	MOV	tmp3, tmp1, LSR #16	; last two bytes
	LDR	tmp1, [src, #-4]!	; previous four bytes
	ORR	tmp3, tmp3, tmp1, LSL #16
	STR	tmp3, [dst, #-4]!
	SUBS	n, n, #4
	BGE	%B1			; tmp1 contains two bytes
...
0	MOV	lr, tmp1, LSR #24	; last byte
	LDMDB	src!, {tmp1, v1, v2, tmp3}
	ORR	lr, lr, tmp3, LSL #8	; word 4
	MOV	tmp3, tmp3, LSR #24
	ORR	tmp3, tmp3, v2, LSL #8	; word 3
	MOV	v2, v2, LSR #24
	ORR	v2, v2, v1, LSL #8	; word 2
	MOV	v1, v1, LSR #24
	ORR	v1, v1, tmp1, LSL #8	; word 1
	STMDB	dst!, {v1, v2, tmp3, lr}
	SUBS	n, n, #16
	BGE	%B0
	; Reload registers
...
1	MOV	tmp3, tmp1, LSR #24	; last byte
	LDR	tmp1, [src, #-4]!	; previous four bytes
	ORR	tmp3, tmp3, tmp1, LSL #8
	STR	tmp3, [dst, #-4]!
	SUBS	n, n, #4
	BGE	%B1			; tmp1 contains one byte
