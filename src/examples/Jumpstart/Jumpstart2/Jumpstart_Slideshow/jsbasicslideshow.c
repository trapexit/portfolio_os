...
/******************************************************************************
**
**  $Id: jsbasicslideshow.c,v 1.16 1995/01/16 19:48:35 vertex Exp $
**
******************************************************************************/

/**
|||	AUTODOC PUBLIC examples/jsbasicslideshow
...
DONE:
    ...
    printf("end of jsbasicslideshow\n");
...
*/
{
	...
	if (argc < 2)
		{
		if ( ParseScriptFile("jsdata/basicimagelist.txt") < 0 )
			goto DONE;
		haveFilename = TRUE;
		}
...
#if DEBUG
	image = LoadImage( filename, gImage[whichBuffer], (VdlChunk **) NULL, gScreenContext);
	if ( image == NULL )
#else
	if ( LoadImage(filename, gImage[whichBuffer], (VdlChunk **) NULL, gScreenContext) == NULL )
#endif
		{
...
*/
{
    ...
    for ( imageIndex = 0; imageIndex < NUM_BUFFERS; imageIndex++ )
		{
		gImage[imageIndex] = (ubyte *)AllocMem(
						(int32)(gScreenContext->sc_nFrameByteCount),
						MEMTYPE_STARTPAGE | MEMTYPE_VRAM | MEMTYPE_CEL);
	
		if ( gImage[imageIndex] == NULL )
			{
			PRT( ("Can't allocate gImage") );
			goto DONE;
			}
		/* initialize it to black */
		SetVRAMPages( gVRAMIOReq, gImage[imageIndex], 0, gScreenContext->sc_nFrameBufferPages, -1);
		}
	}
...
/*	Show the user the acceptable command line formats */
{
	...
	PRT( ("   \"jsdata/basicimagelist.txt\" is used if none is specified.\n") );
    PRT( ("   options:\n") );
